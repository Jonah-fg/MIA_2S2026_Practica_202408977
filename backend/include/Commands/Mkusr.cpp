...
namespace Commands{
    ...
    CommandResult Mkusr_Command(const vector<string>& tokens) {
        ...
        for (const auto& param : encontrados) {
            ...
            if (clave == "-user") {
                if (valor.length() > 10) {
                    return {false, "ERROR: El nombre de usuario excede los 10 caracteres permitidos"};
                }
                userVal =valor;
                tieneUser =true;
            }
                ...
                if (valor.length()>10){
                    return {false, "ERROR: La contraseña excede los 10 caracteres permitidos"};
                }
                passVal=valor;
                tienePass = true;
            }
        ...
        if (!tieneUser){
            return {false, "ERROR: Falta el parámetro obligatorio -user en MKUSR"};
        }
        if (!tienePass){
            return {false, "ERROR: Falta el parámetro obligatorio -pass en MKUSR"};
        }
        if (!tieneGrp) {
            return {false,"ERROR: Falta el parámetro obligatorio -grp en MKUSR"};
        }
        string mensaje= "MKUSR ejecutado correctamente. Usuario: " + userVal+  ", Contraseña: " + passVal + ", Grupo: " + grpVal;
        return {true, mensaje};
    }
